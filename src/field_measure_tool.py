...
class ROI(GraphicsObject):
    ...
    **Signals**
    ...
    def addHandle_measure(self, info, index=None):
        ## If a Handle was not supplied, create it now
        if 'item' not in info or info['item'] is None:
            h = Handle_measure(self.handleSize, typ=info['type'], pen=self.handlePen, parent=self)
            h.setPos(info['pos'] * self.state['size'])
            info['item'] = h
        else:
            h = info['item']
    ...
    def removeHandle(self, handle):
        ...
        if len(handle.rois) == 0 and self.scene():
    ...
    def mouseClickEvent(self, ev):
        ...
        if ev.button() == QtCore.Qt.RightButton and self.isMoving:
            ...
            if ev.button() == QtCore.Qt.RightButton and self.deletable:
                #% remove the tool handles when right clicked
                if isinstance(self.parent, measureTool):
                    self.parent.clearPoints()
                elif isinstance(self.parent, areaMeasureTool):
                    #% cannot calculate area of less than 4 handles
                    if len(self.parent.handles)>3:
...
class Handle_measure(Handle):
    ...
    def paint(self, p, opt, widget):
        p.setRenderHints(p.Antialiasing, True)
        p.setPen(pg.functions.mkPen((255,255,255), width=1))       
        p.drawPath(self.shape())
...
#     def paint(self, p, opt, widget):
#         p.setRenderHints(p.Antialiasing, True)
#         p.setPen(pg.functions.mkPen('#FFFFFF', width=1))
#         p.drawPath(self.shape())
...
class measureTool(PolyLineROI):
    ...
    def mouseClickEvent(self, ev):
        ...
        self.clearPoints()
        self.setPoints([[view_point.x(),view_point.y()], [view_point.x(),view_point.y()]])
    ...
    def addHandle(self, info, index=None):
        h = ROI.addHandle_measure(self, info, index=index)
        h.sigRemoveRequested.connect(self.removeHandle)
        self.stateChanged(finish=True)
        return h

    def addSegment(self, h1, h2, index=None):
        seg = _PolyLineSegment_measure(handles=(h1, h2), pen=self.pen, parent=self, movable=False)
        if index is None:
            self.segments.append(seg)
        else:
            self.segments.insert(index, seg)
...
class LineSegmentROI_measure(ROI):    
    def __init__(self, positions=(None, None), pos=None, handles=(None,None), **args):
        if pos is None:
            pos = [0,0]
    ...
    def paint(self, p, *args):
        p.setRenderHint(QtGui.QPainter.Antialiasing)
        p.setPen(pg.functions.mkPen((255,255,255), width=1, dash=[4, 2]))
        h1 = self.handles[0]['item'].pos()
        h2 = self.handles[1]['item'].pos()
        p.drawLine(h1, h2)
...
class _PolyLineSegment_measure(LineSegmentROI_measure):
    # Used internally by PolyLineROI
    def __init__(self, *args, **kwds):
        self._parentHovering = False
        LineSegmentROI_measure.__init__(self, *args, **kwds)

class areaMeasureTool(PolyLineROI):
    sigClicked = QtCore.Signal(object, object)
    ...
    def finishPattern(self):
        # % delete the last handle (which was being updated dynamically), as we don't need it any longer, and connect the last point to the first line
        self.setPoints([[x['pos'].x(), x['pos'].y()] for x in self.handles[:-1]]+[[self.handles[0]['pos'].x(), self.handles[0]['pos'].y()]])

        # % finish the scan pattern by reseting the active tool
        x = np.array([x[1].x() for x in self.getLocalHandlePositions()])
        y = np.array([x[1].y() for x in self.getLocalHandlePositions()])
        surface_area_um = 0.5 * np.abs(np.dot(x, np.roll(y, 1)) - np.dot(y, np.roll(x, 1)))
        self.parentItem().getViewBox()._parent._parent.statusUpdate("Polygon encloses a surface area of {:.4f} mm2".format(surface_area_um/1e6))
        # self.parentItem().getViewBox().activeScanTool = 0
        self.areaFinished=1